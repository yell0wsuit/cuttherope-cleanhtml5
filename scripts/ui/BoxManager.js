define("ui/BoxManager", [
  ...
  var BoxManager = new (function () {
    ...
    this.isNextLevelPlayable = function () {
      ...
      if (
        ScoreManager.levelCount(this.currentBoxIndex) <= this.currentLevelIndex
      ) {
        return false;
      }
      ...
      ); // NOTE: checking next level since this index is 1 based (TODO: fix!)
    };
    ...
    this.updateBoxLocks = function () {
      ...
      for (boxIndex = 1; boxIndex < numBoxes; boxIndex++) {
        box = boxes[boxIndex];
        if (
          !ScoreManager.isBoxLocked(boxIndex) &&
          box.purchased &&
          box.islocked
        ) {
          box.islocked = false;
