define("visual/Text", [
  ...
  Text = BaseElement.extend({
    ...
    updateDrawerValues: function () {
      ...
      if (this.formattedStrings.length <= 1) {
        ...
        this.height =
          (this.font.fontHeight() + this.font.lineOffset) *
            this.formattedStrings.length -
    ...
    formatText: function () {
      ...
      while (dx < len) {
        ...
        if (
          (MathHelper.roundP2(wp + wc) > this.wrapWidth && xpe != xp) ||
          c == "\n"
        ) {
          strIdx[idx++] = xp;
          strIdx[idx++] = xpe;
  ...
  Text.drawSystem = function (options) {
    ...
    cnv.width =
      options.width || options.maxScaleWidth || options.text.length * 16;
    ...
    if (
      ...
      (options.width && metric.width > options.width)
    ) {
