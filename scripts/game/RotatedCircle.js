define("game/RotatedCircle", [
  ...
  var RotatedCircle = BaseElement.extend({
    ...
    setSize: function (value) {
      ...
      var newStickerScale =
        newScale >= STICKER_MIN_SCALE ? newScale : STICKER_MIN_SCALE;
      ...
      this.vinilCenter.scaleX =
        1.0 - (1.0 - newStickerScale) * CENTER_SCALE_FACTOR;
      this.vinilCenter.scaleY = this.vinilCenter.scaleX;

      this.sizeInPixels =
        this.vinilHighlightL.width * this.vinilHighlightL.scaleX;
    ...
    draw: function () {
      ...
      if (this.isRightControllerActive() || this.isLeftControllerActive()) {
        var lineWidth =
            (ACTIVE_CIRCLE_WIDTH + resolution.PM) *
            this.vinilControllerL.scaleX,
    ...
    drawCircleIntersection: function (
      cx1,
      cy1,
      radius1,
      cx2,
      ...
      if (
        circleDistance >= radius1 + radius2 ||
    ...
    updateChildPositions: function () {
      ...
      var highlightDeltaX =
          (this.vinilHighlightL.width / 2) *
          (1.0 - this.vinilHighlightL.scaleX),
        highlightDeltaY =
          (this.vinilHighlightL.height / 2) *
          (1.0 - this.vinilHighlightL.scaleY),
        controllerDeltaX =
          this.sizeInPixels -
          (CONTROLLER_SHIFT_PARAM1 - CONTROLLER_SHIFT_PARAM2 * this.size) +
          (1.0 - this.vinilControllerL.scaleX) *
            (this.vinilControllerL.width / 2);

      this.vinilHighlightL.x = this.x + highlightDeltaX;
      this.vinilHighlightR.x = this.x - highlightDeltaX;
      this.vinilHighlightL.y = this.vinilHighlightR.y =
        this.y - highlightDeltaY;
    ...
    containsSameObjectWithCircle: function (anotherCircle) {
      ...
      for (i = 0; i < len; i++) {
        if (
          anotherCircle.containedObjects.indexOf(this.containedObjects[i]) >= 0
        ) {
          return true;
        }
      }
