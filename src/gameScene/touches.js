...
const touchMethods = {
    touchDown(x, y, touchIndex) {
        ...
        if (this.gravityButton) {
            ...
            const child = this.gravityButton.getChild(childIndex);
        ...
        if (this.candyBubble && this.handleBubbleTouch(this.star, x, y)) {
            return true;
        }

        if (this.twoParts !== PartsType.NONE) {
            if (this.candyBubbleL && this.handleBubbleTouch(this.starL, x, y)) {
                return true;
            }
            if (this.candyBubbleR && this.handleBubbleTouch(this.starR, x, y)) {
                return true;
            }
        }
        ...
        for (let i = 0, len = this.rotatedCircles.length; i < len; i++) {
            ...
            const d1 = Vector.distance(cameraAdjustedX, cameraAdjustedY, r.handle1.x, r.handle1.y);
            const d2 = Vector.distance(cameraAdjustedX, cameraAdjustedY, r.handle2.x, r.handle2.y);
            if (
        ...
        for (let i = 0, len = this.bungees.length; i < len; i++) {
            ...
            if (grab.wheel) {
                if (grab.wheel.onTouchDown(cameraAdjustedX, cameraAdjustedY)) {
        ...
        if (this.clickToCut) {
            ...
            const bungee = grab ? grab.rope : null;
    ...
    touchMove(x, y, touchIndex) {
        ...
        for (let i = 0, len = this.rotatedCircles.length; i < len; i++) {
            ...
            if (r.operating === touchIndex) {
                ...
                let m2 = Vector.subtract(cameraAdjustedTouch, c);
                ...
                for (let j = 0, lenBungees = this.bungees.length; j < lenBungees; j++) {
                ...
                for (let j = 0, lenPumps = this.pumps.length; j < lenPumps; j++) {
                    const g = this.pumps[j];
                    const gn = new Vector(g.x, g.y);
                    if (gn.distance(c) <= r.sizeInPixels + 5 * this.PM) {
                        gn.rotateAround(a, r.x, r.y);
                        g.x = gn.x;
                ...
                for (let j = 0, lenBubbles = this.bubbles.length; j < lenBubbles; j++) {
                    const g = this.bubbles[j];
                    const gn = new Vector(g.x, g.y);
                    if (
        ...
        if (this.dragging[touchIndex]) {
            ...
            currentCuts.push(fc);
            for (let i = 0, len = currentCuts.length; i < len; i++) {
