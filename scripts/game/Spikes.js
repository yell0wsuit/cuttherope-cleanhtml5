define("game/Spikes", [
  ...
  var Spikes = CTRGameObject.extend({
    init: function (px, py, width, angle, t) {
      ...
      if (t > 0) {
        ...
        var normalQuad = IMG_OBJ_ROTATABLE_SPIKES_BUTTON_button_1 + (t - 1) * 2,
          pressedQuad =
            IMG_OBJ_ROTATABLE_SPIKES_BUTTON_button_1_pressed + (t - 1) * 2,
        ...
        this.rotateButton.anchor = this.rotateButton.parentAnchor =
          Alignment.CENTER;
        ...
        var buttonTexture = bup.texture,
          ...
          vo2 = new Vector(
            buttonTexture.preCutSize.x,
            buttonTexture.preCutSize.y,
          );
    ...
    update: function (delta) {
      ...
      if (this.electro) {
        if (this.electroOn) {
          this.electroTimer = Mover.moveToTarget(
            this.electroTimer,
            0,
            1,
            delta,
    ...
    rotateSpikes: function () {
      ...
      var rDelta = this.spikesNormal ? 90 : 0,
        ...
        tl = new Timeline();
      tl.addKeyFrame(
        KeyFrame.makeRotation(this.rotation, KeyFrame.TransitionType.LINEAR, 0),
      );
      tl.addKeyFrame(
