define("resources/ResScale", ["core/Vector", "core/Rectangle"], function (
  Vector,
  Rectangle,
) {
    ...
    scaleOffsets: function (info, scale) {
      ...
      for (i = 0, len = oldOffsets.length; i < len; i++) {
        ...
        if (!ALLOW_OFFSET_ADJUSTMENT || info.skipOffsetAdjustment) {
          ...
          info.adjustmentMaxX = Math.max(
            info.adjustmentMaxX,
            Math.ceil(adjustment.x),
          );
          info.adjustmentMaxY = Math.max(
            info.adjustmentMaxY,
            Math.ceil(adjustment.y),
          );
