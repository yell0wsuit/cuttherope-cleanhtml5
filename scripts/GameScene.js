define("GameScene", [
  ...
  function applyStarImpulse(star, rd, yImpulse, delta) {
    star.applyImpulse(
      new Vector(-star.v.x / rd, -star.v.y / rd + yImpulse),
      delta,
    );
  }
  ...
  var GameScene = BaseElement.extend({
    init: function () {
      ...
      this.camera = new Camera2D(
        resolution.CAMERA_SPEED,
        Camera2D.SpeedType.DELAY,
      );
    ...
    show: function () {
      ...
      bubbleDisappear.getTimeline(a).onFinished =
      ...
      this.candyBubbleAnimation.initTextureWithId(
        ResourceId.IMG_OBJ_BUBBLE_FLIGHT,
      );
      this.candyBubbleAnimation.x = this.candy.x;
      this.candyBubbleAnimation.y = this.candy.y;
      this.candyBubbleAnimation.parentAnchor =
        this.candyBubbleAnimation.anchor = Alignment.CENTER;
      ...
      if (this.twoParts !== PartsType.NONE) {
        ...
        this.candyBubbleAnimationL.initTextureWithId(
          ResourceId.IMG_OBJ_BUBBLE_FLIGHT,
        );
        this.candyBubbleAnimationL.parentAnchor =
          this.candyBubbleAnimationL.anchor = Alignment.CENTER;
        ...
        this.candyBubbleAnimationR.initTextureWithId(
          ResourceId.IMG_OBJ_BUBBLE_FLIGHT,
        );
        this.candyBubbleAnimationR.parentAnchor =
          this.candyBubbleAnimationR.anchor = Alignment.CENTER;
    ...
    startCamera: function () {
      ...
      if (this.mapWidth > SCREEN_WIDTH || this.mapHeight > SCREEN_HEIGHT) {
        ...
        var startX,
          startY,
          cameraTarget =
            this.twoParts !== PartsType.NONE ? this.starL : this.star;
        ...
        var xScroll = cameraTarget.pos.x - SCREEN_WIDTH / 2,
          ...
          targetX = MathHelper.fitToBoundaries(
            xScroll,
            0,
    ...
    loadTutorialImage: function (item) {
      ...
      if (s.special === 0) {
        ...
        tl2.addKeyFrame(
          KeyFrame.makePos(s.x, s.y, KeyFrame.TransitionType.LINEAR, 0),
        );
        tl2.addKeyFrame(
          KeyFrame.makePos(s.x, s.y, KeyFrame.TransitionType.LINEAR, 0.5),
        );
        tl2.addKeyFrame(
          KeyFrame.makePos(s.x, s.y, KeyFrame.TransitionType.LINEAR, 1),
        );
        tl2.addKeyFrame(
          KeyFrame.makePos(
    ...
    loadTarget: function (item) {
      ...
      target.addAnimationSequence(
        ...
        0.05,
        Timeline.LoopType.NO_LOOP,
        (IMG_CHAR_ANIMATIONS_idle3_end - IMG_CHAR_ANIMATIONS_idle3_start + 1) *
          2,
        [
          109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119, 120, 121, 122,
          123, 124, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119, 120,
          121, 122, 123, 124,
        ],
      );
      target.addAnimationEndpoints(
      ...
      this.support = ImageElement.create(
    ...
    update: function (delta) {
      ...
      this.ropesAtOnceTimer = Mover.moveToTarget(
        this.ropesAtOnceTimer,
        0,
        1,
        delta,
      );
      ...
      if (
        !(this.freezeCamera && this.camera.type === Camera2D.SpeedType.DELAY)
      ) {
      ...
      if (this.camera.type === Camera2D.SpeedType.PIXELS) {
        ...
        var starDistance = this.camera.pos.distance(
          new Vector(targetX, targetY),
        );
        ...
        if (this.fastenCamera) {
          ...
          if (starDistance > this.initialCameraToStarDistance / 2.0) {
            ...
            this.camera.speed = Math.min(
              MAX_PREVIEW_CAMERA_SPEED,
              this.camera.speed,
            );
          } else {
            this.camera.speed -= delta * PREVIEW_CAMERA_SPEED2;
            this.camera.speed = Math.max(
              MIN_PREVIEW_CAMERA_SPEED,
              this.camera.speed,
            );
          }
        }
        ...
        for (i = 0, len = this.stars.length; i < len; i++) {
          ...
          if (s.timeout > 0 && s.time === 0) {
            ...
            if (hits) {
              ...
              this.stars[i] = null;
              SoundMgr.playSound(
                ResourceId.SND_STAR_1 + this.starsCollected - 1,
              );
      ...
      for (i = 0, len = this.socks.length; i < len; i++) {
        s = this.socks[i];
        s.update(delta);
        var moveStatus = Mover.moveToTargetWithStatus(
          s.idleTimeout,
          0,
          1,
          delta,
      ...
      for (i = 0, len = this.spikes.length; i < len; i++) {
        ...
        if (!s.electro || s.electroOn) {
          ...
          if (this.twoParts !== PartsType.NONE) {
            candyHits =
              !this.noCandyL && isCandyHit(s, this.starL, star_spike_radius);
            if (candyHits) {
              left = true;
            } else {
              candyHits =
            ...
            candyHits =
              !this.noCandy && isCandyHit(s, this.star, star_spike_radius);
          }
          ...
          if (candyHits) {
            ...
            var candyTexture = ResourceMgr.getTexture(
      ...
      for (i = 0, len = this.bouncers.length; i < len; i++) {
        ...
        if (this.twoParts !== PartsType.NONE) {
          candyHits =
            !this.noCandyL && isCandyHit(bouncer, this.starL, bouncer_radius);
          if (candyHits) {
            left = true;
          } else {
            candyHits =
          ...
          candyHits =
            !this.noCandy && isCandyHit(bouncer, this.star, bouncer_radius);
        }
      ...
      var gravityMultiplier =
          this.gravityButton && !this.gravityNormal ? -1 : 1,
    ...
    gameWon: function () {
      ...
      tl.addKeyFrame(
        KeyFrame.makeScale(0, 0, KeyFrame.TransitionType.LINEAR, 0.1),
      );
      tl.addKeyFrame(
        KeyFrame.makeColor(
          RGBAColor.solidOpaque.copy(),
    ...
    operatePump: function (pump, delta) {
      pump.playTimeline(0);
      var soundId = MathHelper.randomRange(
    ...
    spiderBusted: function (g) {
      ...
      tl.addKeyFrame(
        KeyFrame.makeRotation(MathHelper.randomRange(-120, 120), 0, 1),
      );
      s.addTimelineWithID(tl, 0);
      s.playTimeline(0);
      s.x = g.spider.x;
    ...
    touchDown: function (x, y, touchIndex) {
      ...
      if (
        ...
        fadeOut.onFinished = $.proxy(
          this.onRotatedCircleTimelineFinished,
          this,
        );
