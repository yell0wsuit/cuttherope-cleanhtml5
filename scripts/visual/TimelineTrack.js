    ...
    updateActionTrack: function (delta) {
      if (this.state === TrackState.NOT_ACTIVE) {
        if (!this.t.timelineDirReverse) {
          if (
            !(
              this.t.time - delta > this.endTime || this.t.time < this.startTime
            )
          ) {
          ...
          if (
            !(
              this.t.time + delta < this.startTime || this.t.time > this.endTime
            )
          ) {
            if (this.keyFrames.length > 1) {
              this.state = TrackState.ACTIVE;
      ...
      if (
        ...
        switch (this.type) {
          ...
          case TrackType.ROTATION:
            var rDelta =
                this.currentStepAcceleration.value.rotationAngle * delta,
        ...
        case TrackType.ROTATION:
          if (!this.relative) {
            ...
            this.t.element.rotation =
              this.elementPrevState.value.rotationAngle +
      ...
      switch (this.type) {
        ...
        case TrackType.ROTATION:
          spsValue.rotationAngle =
            (dst.value.rotationAngle - src.value.rotationAngle) /
            this.keyFrameTimeLeft;
          break;
        case TrackType.COLOR:
      ...
      if (isEaseIn || isEaseOut) {
        switch (this.type) {
          ...
          case TrackType.ROTATION:
            ...
            saValue.rotationAngle =
              spsValue.rotationAngle / this.keyFrameTimeLeft;
            if (isEaseIn) {
